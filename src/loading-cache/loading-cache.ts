...
//         recordBS$.pipe(
//           // set$ can update record during load
//           skip(1),
//           mapTo(null)
//         )
//       )
//         .pipe(timeout(this._timeout), first())
//         .subscribe(
//           loadResultOrNull => {
//             if (loadResultOrNull) {
...
//       filter(
//         recordLocal =>
//           !this._isRecordSpoiled(recordLocal) || !recordLocal.isLoading
//       ),
//       map(recordLocal => {
...
//   }
//
//   public set$(key: K, value: V): Observable<V> {
//     const recordBS$ = this._getOrRegisterRecordBS$(key);
//     const record = recordBS$.getValue();
//
...
//   ): boolean {
//     return (
//       loadResult.timestamp > record.valueTimestamp &&
//       !(Date.now() - loadResult.timestamp > this._spoilTime)
//     );
//   }
//
//   private _isRecordSpoiled(record: ILoadingCacheRecord<V>): boolean {
//     return Date.now() - record.valueTimestamp > this._spoilTime;
//   }
//
//   private _shouldRefreshRecord(record: ILoadingCacheRecord<V>): boolean {
