export function computeLevenshteinDistance(
  ...
  for (
    let longerStringIndex = 1;
    longerStringIndex <= longerString.length;
    longerStringIndex++
  ) {
    newCostList[0] = longerStringIndex;
    for (
      let shorterStringIndex = 1;
      shorterStringIndex <= shorterString.length;
      shorterStringIndex++
    ) {
      newCostList[shorterStringIndex] = Math.min(
        costList[shorterStringIndex - 1] +
          (longerString[longerStringIndex - 1] !==
          shorterString[shorterStringIndex - 1]
            ? 1
            : 0), // substitution
